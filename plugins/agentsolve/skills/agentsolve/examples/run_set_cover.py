#!/usr/bin/env python3
"""Run a weighted set-cover quote -> job -> poll example."""
...
PROBLEM_TYPE = "9.2.set_cover"
INPUT_SCHEMA_VERSION = "9.2.set_cover.input.v3"
OUTPUT_SCHEMA_VERSION = "9.2.set_cover.output.v1"
EXAMPLE_ID = "set-cover-seeded-001"
CANONICAL_PROBLEM_HASH = "dd2bc3d24fb957cf8f43dcd7658f9a6bd2c42fb9c9cbeeb6c4f96c3f671c6727"
...
# ``audit`` carries an ``uncovered_penalty``: covering it costs 5, but leaving it
# short costs only 2, so the exact optimum declines to cover it and pays the
# penalty. Elements without a penalty stay hard covers.
PAYLOAD = {
    "elements": [
        {"id": "accounting"},
        {"id": "security"},
        {"id": "audit", "uncovered_penalty": 2},
    ],
    "sets": [
        {"id": "accounting-only", "cost": 1, "covers": ["accounting"]},
        {"id": "security-only", "cost": 1, "covers": ["security"]},
        {"id": "audit-only", "cost": 5, "covers": ["audit"]},
    ],
}
...
DRY_RESULT = {
    ...
    "selected_set_ids": ["accounting-only", "security-only"],
    "total_cost": 4,
    "best_bound": 4,
    "proved_optimal": True,
    "warnings": [],
